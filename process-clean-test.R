...
# all dataset
df <- labv_clean_2 |> left_join(biomarkers)
...
# ============ Extra check for correctness ============

bmi_diff <- df_diff |> 
  arrange(Sample.id) |> 
  pull(BMI)

bmi_diff2 <- labv_clean |> 
  arrange(`Participant ID`) |> 
  mutate(BMI = `BMI T1` - `BMI T0`) |>
  pull(BMI)

# IDs are the same when sorted
(labv_clean$`Participant ID` |> sort()) == (df_diff$Sample.id |> sort())

# diff calculation was correct
bmi_diff == round(bmi_diff2, 6)
